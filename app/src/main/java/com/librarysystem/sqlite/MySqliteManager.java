...
public class MySqliteManager extends SQLiteOpenHelper {
    ...
    private static final String CREATE_PERSONALMES="create table PersonalMessages(user_id integer primary" +
            ...
            "user_tel text,user_level text,user_pastbooks text,user_wpastbooks text,rootmanager text)";
    /*
    account建表语句
     */
   ...
   repertory建表语句
    ...
    private static final String CREATE_REPERTORY="create table BookRepertory(book_id integer primary" +
            "key ,book_name text,book_author text,book_version text,book_press,book_status text,back_time text,book_description text,book_subscribe text)";
    /*
    present建表语句
     */
    private static final String CREATE_PRESENT="create table PresentBooks(book_id integer primary" +
            "key ,reader_id integer ,book_name text,book_author text,book_version text,book_press,book_description text,borrow_start text,back_time text,book_subscribe text)";
...
private static final String CREATE_USER="insert into PersonalMessages(user_id,user_password,user_name," +
        ...
        "user_level,user_pastbooks,user_wpastbooks,rootmanager)values(?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(CREATE_USER,new String[]{"12345","12345","老宋","男","电子信息","心理学，刑侦学，科普，书画，文学等","18814126594","5","0","0","管理员"});
